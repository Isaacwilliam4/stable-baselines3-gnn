...
class MultiBasePolicy(BaseModel, ABC):
    """The base policy object.
...
class ActorCriticFlowPolicy(BasePolicy):
    """
    Policy class for actor-critic algorithms (has both policy and value prediction).
    Used by A2C, PPO and the likes.
...
class ActorCriticEdgePolicy(ActorCriticPolicy):
    ...
    def __init__(
        ...
        use_expln: bool = False,
        squash_output: bool = False,
        features_extractor_class: GNNEdgeExtractor = GNNEdgeExtractor,
        features_extractor_kwargs: Optional[dict[str, Any]] = None,
...
class ActorCriticNodePolicy(BasePolicy):
    """
    Policy class for actor-critic algorithms (has both policy and value prediction).
    Used by A2C, PPO and the likes.
    ...
    def __init__(
        ...
        use_expln: bool = False,
        squash_output: bool = False,
        features_extractor_class: dict[str, BaseFeaturesExtractor] = None,
        features_extractor_kwargs: Optional[dict[str, Any]] = None,
        share_features_extractor: bool = False,
        normalize_images: bool = True,
        optimizer_class: type[th.optim.Optimizer] = th.optim.Adam,
...
#             lr_schedule=lr_schedule,  # Passed explicitly only once
#             net_arch=net_arch,
...
#     def _build_mlp_critic(self) -> None:
#         """
#         Create the policy and value networks.
#         Part of the layers can be shared.
#         """
...
#         log_prob = distribution.log_prob(actions)
#         actions = actions.reshape((-1, *self.action_space.shape))  # type: ignore[misc]
#         topv, topi = th.topk(actions, self.max_extracted, dim=1)
#         final_actions = self.policy2(actions_max)
...
class ActorCriticGNNPolicy(ActorCriticPolicy):
    ...
    def __init__(
        ...
        use_expln: bool = False,
        squash_output: bool = False,
        features_extractor_class: type[BaseFeaturesExtractor] = GNN,
        features_extractor_kwargs: Optional[dict[str, Any]] = None,
        share_features_extractor: bool = True,
        normalize_images: bool = True,
        optimizer_class: type[th.optim.Optimizer] = th.optim.Adam,
