...
from stable_baselines3.common.multi_on_policy_algorithm import MultiOnPolicyAlgorithm
...
SelfPPO = TypeVar("SelfPPO", bound="MultiActorPPO")


class MultiActorPPO(MultiOnPolicyAlgorithm):
    """
    ...
    def __init__(
        self,
        policy: Union[str, type[ActorCriticPolicy], dict[str, BasePolicy]],
    ...
    def train(self) -> None:
        ...
        for key, _policy in self.policies.items():
            ...
            self._update_learning_rate(_policy.optimizer)
            ...
            for epoch in range(self.n_epochs):
                ...
                for rollout_data in self.rollout_buffers[key].get(self.batch_size):
                    ...
                    values, log_prob, entropy = _policy.evaluate_actions(rollout_data.observations, actions)
                    ...
                    # Optimization step
                    _policy.optimizer.zero_grad()
                    loss.backward()
                    # Clip grad norm
                    th.nn.utils.clip_grad_norm_(_policy.parameters(), self.max_grad_norm)
                    _policy.optimizer.step()
